...
export const ConversationHistory: React.FC<ConversationHistoryProps> = ({
  ...
  return (
    <div className="w-full bg-surface border border-edge rounded-2xl p-5 sm:p-6 shadow-hamyar space-y-4">
      {/* Header */}
      <div className="flex items-center justify-between border-b border-edge pb-3">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="inline-flex items-center gap-1 text-xs font-semibold text-ink-muted hover:text-ink bg-page hover:bg-page px-2.5 py-1.5 rounded-lg transition"
          >
          ...
          <button
            ...
            className="p-1.5 text-ink-muted hover:text-fault rounded-lg hover:bg-fault-bg transition"
          >
      ...
      <div className="space-y-3 max-h-96 overflow-y-auto pr-1">
        {history.map((item, idx) => {
          ...
          return (
            <div
              key={item.id || idx}
              className="p-3.5 bg-page hover:bg-page border border-edge rounded-xl transition space-y-2 text-xs"
            >
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <span className="font-bold text-primary bg-page px-2 py-0.5 rounded text-xs">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button
                    ...
                    className="p-1.5 text-ink-muted hover:text-ink hover:bg-surface rounded-lg transition"
                  >
                    {copiedId === item.id ? (
              ...
              {/* Text preview */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1 border-t border-edge">
                <div dir={isFarsi ? 'rtl' : 'ltr'}>
                  <span className="text-xs text-ink-muted uppercase font-bold block mb-0.5">Original:</span>
                  <p className={`text-ink ${isFarsi ? 'font-farsi font-medium' : 'font-sans'}`}>
                    {item.sourceText}
                  </p>
                </div>

                <div>
                  <span className="text-xs text-primary uppercase font-bold block mb-0.5">British Translation:</span>
